...
| # | 내용 | 상태 · 메모 |
|---|---|---|
| **N-1** | **새 기준으로 260908 비교 재현** (`260914_zereco_new_baseline`, 2026-09-14 사용자 결정): `piq_rfp_critical_slice`(새 기준, critical-path 세부 flag 없이 코드 기본값) + `piq_rfp_full_slice`(PUBS식 full slice, 같은 테이블·edge·refresh). 비교 표의 나머지 행: baseline = `260905_critpath_phaseB/baseline_randq`, P-IQ only·RFP only = `260908_critpath_comparison`의 `piq_only_*`/`rfp_only_*` 재사용(260908의 chain 설정), TEA = `260913_TEA` | **descriptor 설정 완료, 실행 대기.** 정합성: `piq_rfp_critical_slice`는 `260911_critpath_edge_conf/crit_A3e1`과 simpoint별로 완전히 같아야 한다(같은 설정이 기본값이 됨). 주의: `260908_critpath_comparison/baseline_randq`는 GC186 baseline 복사본이라 쓰지 않는다 |
