...
| # | 내용 | 상태 · 메모 |
|---|---|---|
| **N-1** | **새 기준 확인** (`260914_zereco_new_baseline`): critical-path 세부 flag 없이 켜는 스위치만 준 config가 `260911_critpath_edge_conf/crit_A3e1`과 simpoint별로 완전히 같아야 한다 — 기본값 전환의 정합성 확인 | **config 교체 필요.** 지금 `zereco_dbg.json`의 config 8개는 옛 A 조정 배치 그대로라, `exempt_depth`를 명시하지 않은 5개(crit_A3, A2, A1, A3d, A2d)는 새 기본값에서 이름과 달리 e1로 돈다 |
